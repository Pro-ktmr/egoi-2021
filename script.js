...
window.onload = function () {
    ...
    req.open("GET", "list.txt?" + new Date().getTime(), false);
    req.send(null);
    ...
    for (var i=0; i<result.length; i++) {
        ...
        if (result[i][1] != "") td1.innerHTML = '<a href="' + result[i][1] + '" target="_blank">' + result[i][0] + '</a>';
        else td1.innerHTML = result[i][0];

        var td2 = tr.insertCell(-1);
        if (result[i][2] != "") td2.innerHTML = '<a href="' + result[i][2] + '" target="_blank">解説</a>';
    ...
    if (atcoder_id.value != "") {
        ...
        for (var k = 0; k < text.length; k++) {
            ...
            for (var i = 0; i < problem_tbody.rows.length; i++) {
                var tmp = 0;
                var str = problem_tbody.rows[i].cells[0].innerHTML.replace(/\r?\n/g, "");
                if (str.match('tasks/.*" ') === null) continue;
